  ...
  const cardContent = (
    <>
      ...
      <div className="-mt-[45px] mb-8 flex items-center gap-2">
        {category && (
          <span
            className={`inline-block shrink-0 whitespace-nowrap px-3 py-1.5 text-xs font-medium rounded-full ${getCategoryColor(category.title)} !text-brand-black`}
          >
            {category.title}
          </span>
        )}
        {isPinned && (
          <span className="inline-flex shrink-0 whitespace-nowrap items-center gap-1 px-3 py-1.5 text-xs font-medium rounded-full bg-amber-100 text-amber-800">
